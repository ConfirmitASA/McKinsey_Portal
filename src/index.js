...
function PreviewEmployeeWording() {
  ...
  for (var i = 0; i < statements.length; ++i) {
    o.push(
      '<li>' +
        statements[i]
          .replace('$wording-employees$', '<span class="highlight-word">' + value + '</span>')
...
function RenderForm(target_id, form_id, definition) {
  ...
  for (var tab_id in definition) {
    ...
    tabs.push(
      '<div class="tile tab" style="font-size: 15px" id="tab-' + tab_id + '">' + tab_html + '</div>'
    );
      ...
      switch (obj.Type) {
        ...
        case 'Dropdown':
          ...
          if (obj.HasPreview) {
            // Preview
            tmp.push(
              '<div style="padding: 20px 0 0 0" id="preview-' + key + '">' + 'Preview' + '</div>'
            );
            // /Preview
...
function LoadProducts() {
  ...
  for (var key in Products) {
    ...
    for (var j = 0; j < product.Numbers.length; ++j) {
      ...
      var sub =
        x.Sub == null ? '' : `<div style="line-height: 16px; margin-top: 4px">${x.Sub}</div>`;
...
function LoadHelp() {
  ...
  for (var i = 0; i < records.length; ++i) {
    ...
    o.push(
      '<div class="project-tile" id="help-tile-' +
        i +
        '" xonclick="javascript:ActivateProjectId(' +
        record.ProjectId +
        ');" class=project>'
    );
    ...
    o.push(
      '<div class="project-image" style="margin-top: 20px; background-image: url(' +
        record.ImageUrl +
        ')"></div>'
    );
